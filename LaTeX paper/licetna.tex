...
Procesoarele prezintă o mare ineficiență când vine vorba de execuția instrucțiunilor care lucrează cu datele din memoria principală. Această ineficiență provine din diferența dintre timpul de execuție aritmetic al procesorului și timpul de acces din memoria pricipală la datele necesare calculului. Diferența dintre aceste două metrici temporale poartă numele de \textit{latență}. Într-adevăr, avansurile tehnologice aduse componentelor digitale de-a lungul decenilor, printre cele mai importante numărându-se îmbunătățirile tehnicii de litografie, au ajutat minimizarea aceste latențe, însă nu destul încât să o elimine din procesul de optimizare a pipeline-ului de execuție.
...
Fie secvența de instrucțiuni RISC-V de acces a datelor din memoria principală prezentată în Figura 19. Se poate observa faptul că procesorul dorește accesul a mai multe spații de adresare consecutive. De fiecare dată când se extrag datele din aceste adrese, unitatea de execuție este obligată să aștepte conform unui timp \textit{$\ {\Delta}$t} egal cu latența de acces a memoriei principale.
 ...
 \label{Figura:40}
 \includegraphics[width=1.0\textwidth]{cachememoryacces.pdf}
 \end{figure}
...
Implementarea VHDL a structurii digitale prezentată mai sus se poate vedea în Figura \ref{Figura:26}.
 ...
 \label{Figura:26}
 \includegraphics[width=0.9\textwidth]{byteadderVHDL.png}
 \end{figure}
...
Entitatea VHDL e unității aritmetice este redată prin codul prezentat în Figura \ref{Figura:26}.
 
 \begin{figure}[h!]
 \centering
 \caption{Entitatea și arhitectura VHDL a unității aritmetice}
 \label{Figura:26}
 \includegraphics[width=0.9\textwidth]{arithmeticengineVHDL.png}
 \end{figure} 
...
Figura \ref{Figura:20} conține schema digitală a motorului logic cât și a elementelor care-l definesc, cele 3 porți logice pe 32 de biți ȘI, SAU, XOR.
 ...
 \label{Figura:20}
 \includegraphics[width=0.8\textwidth]{LogicEngine.png}
 \end{figure}
...
Spre deosebire de calculul aritmetic, operațile logice sunt o trivialitate, lucru redat nu doar prin diagrama digitală, dar și prin codul asociat unității logice, prezent în Figura \ref{Figura:27}.
 ...
 \label{Figura:27}
 \includegraphics[width=0.7\textwidth]{logicengineVHDL.png}
 \end{figure}
...
\begin{tabular}{ ||c|c|c|c|| }
 ...
 \hline
  $\ A \le 0$ & $\ B \le 0$ & $\ -2^{33} \le A-B \le 0$ &   Overflow posibil\\
...
\begin{figure}[h!]
 ...
 \subsubsection{DISPOZITIVE DE MEMORARE}
...
\begin{figure}[h!]
 ...
 \label{Figura:30}
 \includegraphics[width=0.9\textwidth]{instmem.png}
 \end{figure}
...
În Figura \ref{Figura:31} putem observa design-ul hardware generat de sinteza automată pentru memoria de date. Se remarcă multiplexorul \textit{RTL} \textit{MUX}, rolul acestuia fiind de a lega la masă datele care ies din memorie, în cazul în care se dorește scrierea în aceasta, semnalul selector fiind reprezentat de \textit{write enable(we)}.
...
\begin{figure}[h!]
 ...
 \label{Figura:34}
 \includegraphics[width=0.9\textwidth]{regfile.png}
 \end{figure}
...
\subsubsection{EXECUȚIA SINCRONIZATĂ ȘI PROGRAM COUNTER-UL}
...
Figura  \ref{Figura:37} conține codul ce definește un bistabil sincron pe front crescător. Semnalul de reset are o importanță deosebită, initializând contorul și începând astfel execuția instrucțiunilor.
 ...
 \label{Figura:37}
 \includegraphics[width=0.4\textwidth]{bistabilcode.png}
 \end{figure}
...
Codul aferent întregii entități VHDL a program counter-ului se regăsește în Figura \ref{Figura:38}.
 ...
 \caption{Bistabilul sincron ca entitate VHDL}
 \label{Figura:38}
 \includegraphics[width=0.6\textwidth]{programcountercode.png}
 \end{figure}
...
\newpage
\subsubsection{DECODORUL DE INSTRUCȚIUNI}
...
\newpage
 ...
 \begin{figure}[h!]
 \centering
 \caption{Diagrama digitală a decodorului aritmetic}
 \label{Figura:47}
 \includegraphics[width=1.05\textwidth]{decodercode.png}
 \end{figure}
 
 
 Sinteza automată înlocuiește logica combinațională cu memoria ROM, datele necesare fiind stocate la adresele acestei memorii. În funcție de cuvântul instrucțiunii, multiplexorul intern al acestei memorii comută semnalul de ieșire, trimitând astfel componentelor subordonate semnalele de control corecte.
...
Decodorul aritmetic are rolul de a semnala unității de execuție ce tip de operație este cerută de instrucțiune. Tabela \ref{Tabela:17} prezintă modul său de funcționare. ALUControl este semnalul de ieșire al acestei componente spre unitatea aritmetică și logică, specificând multiplexorului acesteia operația cerută.
...
Codul VHDL al decodorului aritmetic este prezentat în Figura  \ref{Figura:46}.
 ...
 \label{Figura:46}
 \includegraphics[width=0.78\textwidth]{aludecodercode.png}
 \end{figure}

\newpage

Entitatea logică generată de sinteza VHDL pentru decodorul aritmetic se poate vedea în Figura \ref{Figura:45}.
 
  \begin{figure}[h!]
 \centering
 \caption{Entitatea VHDL a decodorului aritmetic}
 \label{Figura:46}
 \includegraphics[width=0.78\textwidth]{decoder.png}
 \end{figure}
